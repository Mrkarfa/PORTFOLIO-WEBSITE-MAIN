...
export default function Footer() {
  ...
  // Sticky footer reveal effect - track scroll progress
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start end", "end end"],
  });

  // Smooth scale transform - footer scales from 0.8 to 1 as it comes into view
  const scale = useTransform(scrollYProgress, [0, 0.5, 1], [0.8, 0.95, 1]);

  // Smooth border radius transform - creates revealing effect
  const borderRadius = useTransform(
    scrollYProgress,
    [0, 0.5, 1],
    ["50px 50px 0px 0px", "30px 30px 0px 0px", "0px 0px 0px 0px"]
  );

  // Y position transform for subtle upward movement
    ...
    <div ref={containerRef} className="relative h-screen">
      ...
      <div className="sticky bottom-0 h-screen overflow-hidden">
        <motion.footer
          ...
          className="h-full bg-[#1a1a1a] text-white overflow-hidden origin-bottom"
        >
          <div className="container mx-auto px-6 md:px-12 lg:px-16 py-12 md:py-16 lg:py-20 h-full flex flex-col justify-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 lg:gap-16 mb-16 md:mb-24 relative z-10">
              ...
              >
                ...
                >
                  <div className="h-[1px] bg-zinc-500 w-24"></div>
                  <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
                ...
                <ul className="space-y-2">
                  {socialLinks.map((link, index) => (
                    ...
                    >
                      <a
                        href={link.href}
                        className="text-sm md:text-base text-white/70 hover:text-white transition-all duration-300 inline-block hover:translate-x-2"
                      >
                        {link.name}
                      </a>
                    </motion.li>
                ...
                <ul className="space-y-2 text-left md:text-right">
                  {navLinks.map((link, index) => (
                    ...
                    >
                      <a
                        href={link.href}
                        className="text-base md:text-lg font-light text-white/90 hover:text-white transition-all duration-300 inline-block hover:translate-x-2 md:hover:-translate-x-2"
                      >
                        {link.name}
                      </a>
                    </motion.li>
